...
function matchJobReference(jobs, reference, predicate = () => true) {
  ...
  throw new Error(`No job found for "${reference}". Run /nano:status to list known jobs.`);
}
...
export function buildSingleJobSnapshot(cwd, reference, options = {}) {
  ...
  if (!selected) {
    throw new Error(`No job found for "${reference}". Run /nano:status to inspect known jobs.`);
  }
...
export function resolveResultJob(cwd, reference) {
  ...
  if (active) {
    throw new Error(`Job ${active.id} is still ${active.status}. Check /nano:status and try again once it finishes.`);
  }
