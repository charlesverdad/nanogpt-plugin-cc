...
Run an adversarial NanoGPT review through the shared plugin runtime.
...
- Run:
```bash
node "${CLAUDE_PLUGIN_ROOT}/scripts/nano-companion.mjs" adversarial-review "$ARGUMENTS"
```
...
```typescript
Bash({
  command: `node "${CLAUDE_PLUGIN_ROOT}/scripts/nano-companion.mjs" adversarial-review "$ARGUMENTS"`,
  description: "NanoGPT adversarial review",
  run_in_background: true
})
```
- Do not call `BashOutput` or wait for completion in this turn.
- After launching the command, tell the user: "NanoGPT adversarial review started in the background. Check `/nano:status` for progress."
